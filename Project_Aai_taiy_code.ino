...
void loop() {
  ...
  UpdateFirebase(1, "Top", dateStatus);
}
...
void UpdateFirebase(int id, String name, bool dateStatus) {
  ...
  if (Firebase.Firestore.getDocument(&fbdo, FIREBASE_PROJECT_ID, "", documentPath.c_str(), "")) {
    if (onceStatus) {
      Firebase.Firestore.patchDocument(&fbdo, FIREBASE_PROJECT_ID, "", documentPath.c_str(), content.raw(), (dateStatus) ? "ID, Name, Status, LoginDate" : "ID, Name, Status, LogoutDate");
      onceStatus = false;
    }
    // Serial.printf("ok\n%s\n\n", fbdo.payload().c_str());
  } else {
